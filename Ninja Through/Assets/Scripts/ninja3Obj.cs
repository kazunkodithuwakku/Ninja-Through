...
public class ninja3Obj : MonoBehaviour {
    ...
    void OnMouseDown()
    {
        if (!isObjectTouch)
        {
            ...
            Assets.Scripts.Globals.currentSelectedNinja = 3;
        }
        else
        {
            isObjectTouch = false;
            gameObject.tag = "ninja3";
    ...
    void touchOnObject()
    {
        ...
        if (isObjectTouch && isClickOnPillar)
        {
            ...
            { //&& !(V3Equal(transform.position, endPoint))){
                ...
                Assets.Scripts.Globals.ninja3Location = Assets.Scripts.Globals.MovingPillarID;
            }
