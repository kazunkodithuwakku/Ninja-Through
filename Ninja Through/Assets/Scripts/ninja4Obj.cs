...
public class ninja4Obj : MonoBehaviour {
    ...
    void OnMouseDown()
    {
        if (!isObjectTouch)
        {
            ...
            Assets.Scripts.Globals.currentSelectedNinja = 4;
        }
        else
        {
            isObjectTouch = false;
            gameObject.tag = "ninja4";
    ...
    void touchOnObject()
    {
        ...
        if (isObjectTouch && isClickOnPillar)
        {
            ...
            { //&& !(V3Equal(transform.position, endPoint))){
                ...
                Assets.Scripts.Globals.ninja4Location = Assets.Scripts.Globals.MovingPillarID;
            }
